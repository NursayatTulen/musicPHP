...
@section('title', __('Edit News') . ' — Music Hub')
...
<div class="login-architecture fade-in">
    <div class="card" style="max-width: 600px; width: 100%; padding: 40px;">
        <h2 class="text-gradient" style="margin-bottom: 30px; font-size: 1.8rem;">
            <i class="fas fa-edit"></i> {{ __('Edit Music News') }}
        </h2>

        <form action="{{ route('music-news.update', $news->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="title" class="form-input" style="width: 100%;" value="{{ $news->title }}" required>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="date" name="event_date" class="form-input" style="width: 100%;" value="{{ $news->event_date->format('Y-m-d') }}" required>
            </div>
            ...
            <div style="margin-bottom: 30px;">
                ...
                <textarea name="description" class="form-input" style="width: 100%; height: 120px; resize: none;" required>{{ $news->description }}</textarea>
            </div>

            <div style="display: flex; gap: 15px;">
                <button type="submit" class="btn btn-primary" style="flex: 1;">
                    <i class="fas fa-save"></i> {{ __('Update News') }}
                </button>
